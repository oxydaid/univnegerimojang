            ...
            <form wire:submit.prevent="submit" class="space-y-6">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                        ...
                        <div class="space-y-1.5">
                            ...
                            <select id="path" wire:model.live="path" class="w-full px-4 py-3 rounded-none border-2 border-slate-900 text-sm focus:border-primary outline-none bg-white">
                                ...
                                <option value="beasiswa">Jalur Beasiswa Ordal (Orang Dalam)</option>
                            </select>
                            @error('path') <span class="text-xs text-rose-600 font-semibold">{{ $message }}</span> @enderror
                        </div>
                    </div>

                    <!-- Beasiswa Ordal Code Input -->
                    @if($path === 'beasiswa')
                        <div class="p-4 bg-amber-50 border-2 border-amber-300 space-y-3">
                            <div class="flex gap-2 text-amber-800 font-bold text-sm font-pixel">
                                ...
                                <span>Perhatian: Anda memilih Jalur Beasiswa Ordal (Orang Dalam)</span>
                            </div>
                            <p class="text-xs text-amber-700 font-medium">
                                Masukkan nama civitas akademika pemberi rekomendasi atau kode akses khusus orang dalam untuk verifikasi.
                            </p>
                            <div class="space-y-1.5 max-w-md">
                                <label for="ordal_code" class="block text-xs font-bold text-slate-700">Kode Rekomendasi / Nama Pejabat Ordal</label>
                                <input type="text" id="ordal_code" wire:model="ordal_code" class="w-full px-3 py-2 rounded-none border-2 border-slate-900 text-xs focus:border-primary outline-none bg-white" placeholder="e.g., REKTOR-STEVE / ORDAL-EMERALD">
                ...
                <div class="space-y-4 pt-4" x-data="{ openExamples: false }">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-3 gap-6">
                        <!-- Skin Minecraft -->
                        <div class="space-y-2 p-4 border border-slate-200 bg-slate-50">
                            <label class="block text-sm font-bold text-slate-800">
                                <i class="fa-solid fa-shirt mr-1.5 text-primary"></i> Skin (.png, max 1MB)
                            </label>
                            <input type="file" wire:model="skin" class="text-xs w-full">
                            <div wire:loading wire:target="skin" class="text-xxs text-primary font-bold"><i class="fa-solid fa-spinner animate-spin"></i> Mengunggah...</div>
                            @if ($skin)
                                <div class="mt-2 text-xxs text-emerald-600 font-bold"><i class="fa-solid fa-circle-check"></i> Skin terunggah!</div>
                            @endif
                            @error('skin') <span class="text-xs text-rose-600 font-semibold block">{{ $message }}</span> @enderror
                        </div>
                        ...
                        <div class="space-y-2 p-4 border border-slate-200 bg-slate-50">
                            ...
                            <input type="file" wire:model="minecraft_stats" class="text-xs w-full">
                            <div wire:loading wire:target="minecraft_stats" class="text-xxs text-primary font-bold"><i class="fa-solid fa-spinner animate-spin"></i> Mengunggah...</div>
                            @if ($minecraft_stats)
                                <div class="mt-2 text-xxs text-emerald-600 font-bold"><i class="fa-solid fa-circle-check"></i> Statistik terunggah!</div>
                            @endif
                            @error('minecraft_stats') <span class="text-xs text-rose-600 font-semibold block">{{ $message }}</span> @enderror
                        </div>
                        ...
                        <div class="space-y-2 p-4 border border-slate-200 bg-slate-50">
                            ...
                            <p class="text-[10px] text-slate-500 leading-tight">
                                Ijazah kelulusan asli dari SMA/SMK/MAN Negeri Mojang.
                            </p>
                            <input type="file" wire:model="certificate" class="text-xs w-full mt-1">
                            <div wire:loading wire:target="certificate" class="text-xxs text-primary font-bold"><i class="fa-solid fa-spinner animate-spin"></i> Mengunggah...</div>
                            @if ($certificate)
                                <div class="mt-2 text-xxs text-emerald-600 font-bold"><i class="fa-solid fa-circle-check"></i> Ijazah terunggah!</div>
                            @endif
                            @error('certificate') <span class="text-xs text-rose-600 font-semibold block">{{ $message }}</span> @enderror
                        </div>
                    </div>

                    <!-- Conditional Prestasi Proof Input -->
                    @if($path === 'prestasi')
                        <div class="p-4 border-2 border-dashed border-primary bg-emerald-50/30 space-y-2">
                            <label class="block text-sm font-bold text-emerald-900">
                                <i class="fa-solid fa-trophy mr-1.5 text-amber-500"></i> Bukti Prestasi Tambahan (.jpg/.png, max 2MB)
                            </label>
                            <p class="text-xxs text-emerald-700 leading-relaxed">
                                Wajib melampirkan screenshot prestasi singleplayer maupun main di server, atau piagam survival hardcore 100 hari.
                            </p>
                            <input type="file" wire:model="achievement_proof" class="text-xs w-full mt-1">
                            <div wire:loading wire:target="achievement_proof" class="text-xxs text-primary font-bold"><i class="fa-solid fa-spinner animate-spin"></i> Mengunggah...</div>
                            @if ($achievement_proof)
                                <div class="mt-2 text-xxs text-emerald-600 font-bold"><i class="fa-solid fa-circle-check"></i> Bukti prestasi terunggah!</div>
                            @endif
